...
These recur while developing an app on Octane. Guard against each one as you write code.
...
- Reload workers after changing code, providers, env, or config. Workers hold the booted app in memory and pick up changes only after `octane:reload`, a restart, or running with `--watch` in local development.
